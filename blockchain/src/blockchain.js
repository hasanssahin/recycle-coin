...
class Transaction {
    ...
    Kategoriler(fromAddress){
        this.fromAddress = fromAddress;
    }

    calculateUserEmailHash() {
        return SHA256(this.fromAddress).toString();
    }
...
class Blockchain {
    ...
    addTransaction(transaction) {
        ...
        if (!transaction.fromAddress || !transaction.toAddress) {
            return new Error('Transaction must include from and to addres');
        }

        if (!transaction.isValid()) {
            return new Error('Cannor add invalid transaction to chain');
        }
